...
def algorithm(horizon,N=5,M=4):
    double_auction = DoubleAuction(N, M, [40,30,20,5,2], [2,3,4,7]) # change this, take input from part 1 valuations
    ...
    for round in range(1,horizon):
        ...
        for j in range(N):
            buyer_bids[j] =  average_buyer_reward[j]+math.sqrt(16*log(round)/n_buyer[j])# write code
        for k in range(M):
            seller_asking_price[k] =  average_seller_reward[k]-math.sqrt(16*log(round)/n_seller[k]) # write code
